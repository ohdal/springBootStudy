...
public class OrderDetailSample extends StudyApplicationTests {
    ...
    private void item(User user) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            Integer idx = items.indexOf(item);
            if (idx > -1) {
                Integer count = itemsCount.get(idx);
                itemsCount.set(idx, count + 1);
            } else {
                itemsCount.add(1);
                items.add(item);
            }
        ...
        for (Item item : items) {
            ...
            Integer count = itemsCount.get(items.indexOf(item));
            ...
            OrderDetail orderDetail = OrderDetail.builder()
                    ...
                    .item(item)
                    .quantity(count)
                    .totalPrice(item.getPrice().multiply(new BigDecimal(count)))
